...
//namespace CoreBox
//{
    Core::Core(int ScreenWidth, int ScreenHeight, const char* GameName, bool ShowFPS)
    {
        ...
        if(!ifs)
        {
            ...
            outfile << "LKBOX\n" << ScreenWidth << endl << ScreenHeight << endl;
            outfile.close();
    ...
    */
        ...
        while (!glfwWindowShouldClose(window))
        {
            ...
            if((t - t0) > 1.0 || frames == 0)
            {
                ...
                if(ShowFPS == true)
                {
                   sprintf(title_string, "%s |  FPS: %.1f", GameName, fps);
                }
                else
                {
                    sprintf(title_string, GameName);
                }
                //sprintf(title_string, TitleFPS, fps);
                glfwSetWindowTitle(window, title_string);
                t0 = t;
                frames = 0;
            }
            frames ++;
            /*if(debug::fpsCon() == true)
            {   
    ...
    void Core::UICore()
    {
        ...
        if(showWindow)
        {
            ...
            if(isHold == false)
            {
                if(ImGui::Begin("Exit", NULL, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoResize))
                {
                    ImGui::Separator();
                    if(ImGui::Button("Ok"))
                    {
